 ...
 * @author vivaxy
 */
var Input = function (element, size, row, col, saveElement, generateElement) {
    this.events = {};
...
Input.prototype.ifMobile = function () {
    var check = false;
    (function (a) {
...
Input.prototype.stopListen = function () {
    this.listening = false;
};
Input.prototype.getPosition = function (x, y) {
    var mappedX = this.size * this.col / this.element.getBoundingClientRect().width * x;
    var mappedY = this.size * this.row / this.element.getBoundingClientRect().height * y;
    var row = Math.floor(mappedY / this.size);
    var col = Math.floor(mappedX / this.size);
    return {
        row: row,
        col: col
    };
};
Input.prototype.message = function (e) {
    //console.log(e);
    if (!this.listening) {
        return;
    }
    e.stopPropagation();
    var touches = this.isMobile ? e.touches : [e];
    var touch = touches[0];
...
Input.prototype.save = function (e) {
    if (!this.listening) {
        return;
    }
    e.stopPropagation();
...
Input.prototype.generate = function (e) {
    if (!this.listening) {
        return;
    }
    e.stopPropagation();
...
Input.prototype.fire = function (event, data) {
